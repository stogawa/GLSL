...
class WebGLApp {
  ...
  setup() {
    ...
    this.gl.clearDepth(1.0); // クリアする際の深度値 @@@ ※1.0が一番遠く、0が一番手前になる。0にすると何も上書きできなくなる。
  ...
  setupGeometry() {
    ...
    // 頂点を格子状に並べ、座標に応じた色を付ける
		// 初期配置は、-1 ~ 1に等間隔で並んだ状態になる
    const COUNT = 100;
    for (let i = 0; i < COUNT; ++i) {
      const x = i / (COUNT - 1); // 0.0 ~ 0.99
      const signedX = x * 2.0 - 1.0; // -1 ~ 1（ほぼ）
      for (let j = 0; j < COUNT; ++j) {
        const y = j / (COUNT - 1); // 0.0 ~ 0.99
        const signedY = y * 2.0 - 1.0; // -1 ~ 1（ほぼ）
  ...
  render() {
    ...
    const rotateAxis  = v3.create(1.0, 0.0, 0.0); // X 軸回転を掛ける → Y軸にするなら v3.create(0.0, 1.0, 0.0);
    const rotateAngle = this.uTime * 0.1;         // 回転角は時間由来
    const m = m4.rotate(m4.identity(), rotateAngle, rotateAxis); //単位行列
    ...
    const near   = 0.1;                                    // ニア・クリップ面までの距離 → 写す手前の位置
    const far    = 10.0;                                   // ファー・クリップ面までの距離 → 写す最後の位置 ※3D CGでは、どこまで映すかを決める
